...
const MessagesPage = () => {
  ...
  // mobile state: whether we are in "mobile" small-screen mode
  const [isMobile, setIsMobile] = useState(typeof window !== 'undefined' ? window.innerWidth < MOBILE_BREAKPOINT : false);
  // when on mobile, control whether we show the conversation list (true) or the chat pane (false)
  // default: show list
  const [mobileShowList, setMobileShowList] = useState(true);

  // handle resize to toggle isMobile
  useEffect(() => {
    const handleResize = () => {
      const mobile = window.innerWidth < MOBILE_BREAKPOINT;
      setIsMobile(mobile);
      // when switching to desktop, ensure list + chat are visible (desktop split)
      if (!mobile) {
  ...
  useEffect(() => {
    ...
    if (targetUid && targetUid !== user.uid) {
      ...
      // on mobile, if an initial target is provided, go directly to chat view
  ...
  const openConversationWith = async (otherUid) => {
    ...
    try {
      ...
      const recProfile = await getUserProfile(otherUid);
      setReceiver({ uid: otherUid, ...recProfile });

      setActiveConv(conv);
      setError('');

      // If we're on mobile, switch to chat view (hide list)
      if (isMobile) setMobileShowList(false);
    } catch (err) {
  ...
  const renderStatus = () => {
    ...
    return (
      <span style={{ color: 'gray', fontSize: '0.875rem', marginTop: '4px' }}>
        Last seen: {status.lastSeen ? new Date(status.lastSeen).toLocaleString() : '—'}
      </span>
    );
  };

  // UI helpers for mobile toggle
  const handleBackToList = () => {
    setMobileShowList(true);
    // Optional: clear activeConv if you want a fresh start: setActiveConv(null);
  };

  // Decide what to render for panels depending on isMobile and mobileShowList
  // If isMobile && mobileShowList => show only LEFT (list)
  // If isMobile && !mobileShowList => show only RIGHT (chat)
  // If !isMobile => show both (split)
  const showListPanel = !isMobile || (isMobile && mobileShowList);
  const showChatPanel = !isMobile || (isMobile && !mobileShowList);
  ...
  return (
    <Container maxWidth={false} disableGutters sx={{ minHeight: '100vh', pb: 0, background: 'linear-gradient(135deg, #f8f4ff 0%, #fff 100%)' }}>
      ...
      <Grid container>
        ...
        {showListPanel && (
          <Grid item xs={12} md={4} sx={{ minHeight: 'calc(100vh - 64px)', borderRight: !isMobile ? '1px solid rgba(122,47,255,0.08)' : 'none', background: 'rgba(255,255,255,0.7)' }}>
        ...
        {showChatPanel && (
          <Grid item xs={12} md={8} sx={{ minHeight: 'calc(100vh - 64px)', display: 'flex', flexDirection: 'column', background: 'linear-gradient(135deg, rgba(122,47,255,0.03) 0%, rgba(255,95,162,0.03) 100%)', p: 0 }}>
            <Paper elevation={0} sx={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'transparent', boxShadow: 'none', p: 0 }}>
              ...
              {/* On mobile show a back button in header */}
              {receiver && (
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, p: 2, borderBottom: '1px solid rgba(122,47,255,0.15)' }}>
                  {isMobile && (
                    <IconButton onClick={handleBackToList} sx={{ mr: 1 }}>
                      <ArrowBackIcon />
                    </IconButton>
                  )}
                  <Avatar src={receiver?.image || ''} sx={{ width: 52, height: 52 }} />
                  <Box>
                    <Typography sx={{ fontSize: '1rem', fontWeight: 600 }}>
                      {receiver?.name || receiver.uid}
                    </Typography>
                    {renderStatus()}
                  </Box>
                </Box>
